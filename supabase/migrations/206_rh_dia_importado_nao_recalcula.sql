-- 206_rh_dia_importado_nao_recalcula.sql
-- Ao conferir a 205 no espelho de junho/2026 apareceu isto: um atestado do dia
-- inteiro (12/06) contava 8h de FALTA, e o Pontomais não tinha cobrado nada
-- (débito 0). Idem "Ausência Justificada" — 30 min de falta onde eles cobraram
-- zero.
--
-- A causa é o Flow RECALCULAR um dia que veio importado. A régua deles difere:
-- creditam 8h fixas de horas normais, jogam o excedente em extra e abonam
-- atestado/ausência justificada no próprio débito. O Flow, sem saber disso,
-- comparava o trabalhado com a carga e inventava falta — em cima de dias que
-- as pessoas já viram e assinaram no sistema antigo.
--
-- Agora: dia com origem = 'pontomais' usa a apuração DELES
-- (imp_normais_min, imp_debito_min/imp_faltantes_min, imp_he50_min,
-- imp_he100_min). Dia do Flow segue com o cálculo do Flow. A justificativa
-- daqueles dias continua visível no `motivo` importado.
--
-- Vale para o espelho e para o fechamento — os dois somavam do mesmo jeito.
--
-- Idempotente.
