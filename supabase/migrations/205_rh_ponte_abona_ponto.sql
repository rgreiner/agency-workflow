-- 205_rh_ponte_abona_ponto.sql
-- "Quem emenda a ponte aparece com falta no espelho." (Rafael, 04/08)
--
-- O calendário (rh_feriado, mig. 162) abona o dia para a ORG INTEIRA. Numa
-- emenda isso está errado: a adesão é opcional e parte do time trabalha —
-- abonar todos zeraria a carga de quem estava no escritório e transformaria o
-- expediente dele em hora extra. Por isso a emenda nunca entrou no calendário.
--
-- A resposta é abonar POR PESSOA, a partir do cadastro que já existe na 204:
-- a ponte vale para quem estava na casa na data e não está na lista de exceção.
-- Uma função só (rh_ponte_abona), consultada pelo espelho e pelo fechamento nos
-- mesmos dois pontos em que o feriado já era consultado.
--
-- NÃO recalcula dia importado: os minutos e o saldo de um dia do Pontomais
-- continuam vindo de lá (imp_*). O que muda é só a CARGA ESPERADA do dia, que
-- é o que fazia a emenda virar falta.
--
-- Idempotente.
  ...
  select exists (
    select 1
     ...
     where p_data between pt.inicio and pt.fim
       -- Quem entrou depois da ponte não emendou nada.
...
$$;
revoke execute on function rh_ponte_abona(uuid, date) from public, anon;
grant  execute on function rh_ponte_abona(uuid, date) to authenticated;
