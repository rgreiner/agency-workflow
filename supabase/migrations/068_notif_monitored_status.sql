-- 068_notif_monitored_status.sql
-- Exceção à regra: 2 status são MONITORADOS pelo cargo mesmo sem associação —
-- 'revisao_interna' (Revisão interna) e 'validacao_midia' (Validação de mídia).
-- Para esses, quem tem o cargo recebe o aviso ao entrar no status, mesmo não
-- sendo responsável. Demais status seguem só para criador/responsável. Idempotente.
...
begin
  ...
  if NEW.to_status = any(v_monitorados) then
    insert into notifications (user_id, org_id, type, activity_id, actor_id, data)
