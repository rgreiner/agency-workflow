-- 047_cadastros_contato.sql
-- Enriquece os cadastros (cliente=workspace, veículo, fornecedor) com blocos de contato
-- repetíveis em jsonb: endereços, telefones, emails, contas bancárias (+ atividade no cliente).
-- Idempotente.

-- ── Colunas jsonb nos 3 cadastros ────────────────────────────
alter table workspaces  add column if not exists atividade        text;
alter table workspaces  add column if not exists enderecos        jsonb not null default '[]'::jsonb;
alter table workspaces  add column if not exists telefones        jsonb not null default '[]'::jsonb;
alter table workspaces  add column if not exists emails           jsonb not null default '[]'::jsonb;
alter table workspaces  add column if not exists contas_bancarias jsonb not null default '[]'::jsonb;

alter table veiculos    add column if not exists enderecos        jsonb not null default '[]'::jsonb;
alter table veiculos    add column if not exists telefones        jsonb not null default '[]'::jsonb;
alter table veiculos    add column if not exists emails           jsonb not null default '[]'::jsonb;
alter table veiculos    add column if not exists contas_bancarias jsonb not null default '[]'::jsonb;

alter table fornecedores add column if not exists enderecos        jsonb not null default '[]'::jsonb;
alter table fornecedores add column if not exists telefones        jsonb not null default '[]'::jsonb;
alter table fornecedores add column if not exists emails           jsonb not null default '[]'::jsonb;
alter table fornecedores add column if not exists contas_bancarias jsonb not null default '[]'::jsonb;

-- ── Cliente (workspace): estende o save do cadastro ──────────
...
begin
  if not exists (
    ...
    where w.id = p_workspace_id and om.user_id = p_user_id and om.role in ('owner','admin','manager')
...
-- ── Veículo: create/update com blocos de contato ─────────────
...
returns void language plpgsql security definer set search_path = public as $$
begin
  if not exists (select 1 from veiculos v join organization_members om on om.org_id=v.org_id where v.id=p_veiculo_id and om.user_id=p_user_id and om.role in ('owner','admin','manager'))
  then raise exception 'Acesso negado'; end if;
  update veiculos set
...
-- ── Fornecedor: create/update com blocos de contato ──────────
