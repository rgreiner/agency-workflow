-- 213_rh_espelho_abono_por_periodo.sql
-- Liga a régua da 212 no espelho e no fechamento, e APOSENTA o que a 208 fazia.
--
-- A 208 zerava todo o saldo negativo de um dia com justificativa aprovada ou
-- abonada. Parecia certo até o caso da Danielle mostrar que não é: o dia dela
-- tinha 33 min de consulta (com declaração), 10 min de atraso na entrada e 3
-- min de volta depois do fim do atendimento — e a régua antiga perdoava os
-- três juntos.
--
-- Agora o abono desconta da CARGA EXIGIDA do dia, só nos minutos que o
-- documento cobre. Tudo o mais volta a contar:
--   esperado = jornada − abono
--   falta    = esperado − trabalhado
--   saldo    = trabalhado − esperado
--
-- Some daqui o `not exists (justificativa aprovada/abonada)` que blindava o dia
-- inteiro contra falta: com a carga já líquida, ele passaria a perdoar duas
-- vezes.
--
-- A linha passa a devolver `abono_min` para a tela poder mostrar de onde veio
-- o número, em vez de exibir um saldo que ninguém consegue reconferir.
--
-- Dia importado do Pontomais segue intocado (mig. 206): a apuração é a deles.
--
-- Idempotente.
