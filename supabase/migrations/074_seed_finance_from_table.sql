-- 074_seed_finance_from_table.sql
-- Seed da config do Financeiro derivado DIRETO de extrato_importado (não depende de
-- reimportar o arquivo). Cria contas (saldo atual = soma assinada dos realizados),
-- centros de custo e categorias (tipo entrada/saída/ambos). NÃO-DESTRUTIVO: só adiciona
-- o que ainda não existe (por nome, case-insensitive). Idempotente.
...
declare
  ...
  rec record;
  v_ord int;
  v_contas int := 0; v_centros int := 0; v_cats int := 0;
  v_centros_cfg jsonb; v_cats_cfg jsonb;
  i int;
  v_cor text; v_tipo text;
begin
  if not exists (
  ...
  loop
    ...
    if not exists (
      ...
      values (p_org_id, rec.nome, v_tipo, round(coalesce(rec.saldo,0),2), v_cor, true, v_ord, p_user_id);
      v_contas := v_contas + 1;
    end if;
    i := i + 1;
  end loop;
