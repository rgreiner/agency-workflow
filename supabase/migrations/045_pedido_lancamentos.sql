-- 045_pedido_lancamentos.sql
-- Pedido de Produção: ao FATURAR, gera os lançamentos a partir das PARCELAS.
-- Tipos de parcela: cliente_paga_fornecedor (NÃO lança), receber_bv (a receber do
-- fornecedor), receber_honorarios (a receber do cliente). Idempotente.

create or replace function gerar_lancamentos_pedido(p_producao_id uuid)
...
-- set_producao_situacao agora gera os lançamentos do pedido ao faturar.
...
begin
  ...
  perform gerar_lancamentos_pedido(p_producao_id);
end; $$;

grant execute on function gerar_lancamentos_pedido(uuid) to anon, authenticated;
