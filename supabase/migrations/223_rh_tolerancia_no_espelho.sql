-- 223_rh_tolerancia_no_espelho.sql
-- "Total deveria ser 8h e não 7:55. O tempo não deveria gerar os -0:05."
-- (Rafael, 07/08, sobre o dia 06/08 da Isadora)
--
-- Caso: jornada 08:30–12:00 · 13:00–17:30 (480 min), marcações
-- 08:39 · 12:07 · 13:07 · 17:34 → 475 min trabalhados, delta −5.
-- O `rh_ponto.saldo_min` gravado JÁ ERA 0 (a tolerância de 10 min absorveu no
-- rh_recalc_ponto desde a mig. 167). Quem mostrava −0:05 era o espelho, que
-- refazia a conta crua (`minutos − carga`) e contrariava o próprio dia.
--
-- Isso REVISA a decisão de 05/08 ("tolerância não entra no saldo, serve só
-- para não sinalizar atraso na tela"). Duas telas com saldos diferentes do
-- mesmo dia é o problema que a 214 já tinha atacado; aqui a régua fica UMA:
--
--   |trabalhado − carga| <= tolerância  ⇒  saldo 0 E carga cheia de normais
--
-- ⚠️ A tolerância é do TOTAL DO DIA, não de cada marcação. Analisar marcação a
-- marcação seria mais permissivo e ilegal: entrar 10 min tarde, sair 10 min
-- cedo p/ o almoço, voltar 10 tarde e sair 10 cedo daria "tudo dentro" com 40
-- min a menos no dia. O art. 58 §1º da CLT é justamente 5 min por marcação com
-- teto de 10 no dia — que é o teto do total. No caso da Isadora os dois
-- caminhos dão o mesmo: o total é −5.
--
-- Horas normais: dia absorvido credita a CARGA CHEIA (480), não os 475
-- marcados. Sem isso o resumo não fecharia — saldo 0 com H.N. 7:55.
-- A MARCAÇÃO REAL NUNCA MUDA: 08:39 continua 08:39 e o trabalhado do dia
-- continua exibindo 7:55. O que a tolerância zera é a cobrança, não o registro.
--
-- Feriado/fim de semana seguem SEM tolerância (não há carga esperada: conta
-- desde o 1º minuto) e dia importado do Pontomais segue congelado.
-- Idempotente.

-- Régua única da tolerância. Sem carga esperada não há o que tolerar.
create or replace function rh_saldo_tolerado(p_trabalhado int, p_carga int, p_tol int)
returns int language sql immutable as $$
  select case
    when coalesce(p_carga, 0) <= 0 then coalesce(p_trabalhado, 0) - coalesce(p_carga, 0)
    when abs(coalesce(p_trabalhado, 0) - p_carga) <= coalesce(p_tol, 0) then 0
    else coalesce(p_trabalhado, 0) - p_carga
  end
$$;
revoke execute on function rh_saldo_tolerado(int, int, int) from public, anon;
grant  execute on function rh_saldo_tolerado(int, int, int) to authenticated;
...
-- ── Fechamento (report da contabilidade) ────────────────────────────────────
