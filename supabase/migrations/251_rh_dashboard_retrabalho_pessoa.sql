-- 251_rh_dashboard_retrabalho_pessoa.sql
-- Painel de RH: "de quem é o trabalho que volta" + reabertura de recorrência fora
-- do retrabalho. Recria rh_dashboard inteira (1 assinatura por RPC — PostgREST).
--
-- Medido em produção em 21/08/2026 (últimos 12 meses, org One a One):
--   · 585 voltas de etapa; 50 delas eram "Concluído → Mídia" com o comentário fixo
--     de recur_activity ("Recorrência: reaberta para o próximo prazo") — reabertura
--     automática de tarefa recorrente, não retrabalho. Saía como a 4ª maior barra.
--     Fica de fora nos dois gráficos.
--   · Das 535 restantes, a regra abaixo atribui ~450 a uma pessoa; ~85 ficam sem
--     dono (Validação de mídia, Briefing, Pendente do cliente — ninguém na tarefa
--     tem o cargo). Essas aparecem como "sem dono", não somem.
--
-- A REGRA (pedida pelo Rafael em 21/08): a volta é de quem responde pela etapa de
-- DESTINO, não de quem devolveu. Voltou para Redação → é da redatora da tarefa;
-- para Design → do diretor de arte. É a mesma régua cargo × status da mig. 112
-- (org_positions.allowed_statuses), aplicada ao to_status:
--   1) quem estava NA tarefa (activity_assignees, já na data da volta) e cujo cargo
--      cobre a etapa de destino. Se 2+ pessoas cobrem, fica só quem ENTREGOU a etapa
--      por último (moveu a tarefa pra frente a partir dela) — desempata 24 de 31.
--      O filtro assigned_at <= changed_at importa: dos 55 casos em que a pessoa
--      entrou depois da volta, 40 entraram mais de 7 dias depois.
--   2) se ninguém na tarefa cobre a etapa, vale quem entregou a etapa por último
--      E tem o cargo dela (facilitador sem cargo não entra — ver memória do Rafael).
--   Não existe nível "qualquer um com o cargo" (o resp_cargo da 112): aqui ele
--   acusaria quem é único no cargo por tarefa em que nunca esteve.
--
-- changed_by da volta (o revisor) continua fora da conta — ele não errou, devolveu.
-- Idempotente.
