/**
 * PDF do fechamento do ponto, em dois formatos (?tipo=):
 *  - 'resumo' (default): a TABELA do banco de horas, uma linha por pessoa —
 *    o formato que a contabilidade pediu (26/08).
 *  - 'espelho': o documento detalhado, uma pessoa por página, cada uma no
 *    período PRÓPRIO da sua linha (desligado estica até a demissão, mig. 256).
 *
...
import { FechamentoResumoDoc, type FechResumoLinha } from '@/lib/pdf/FechamentoResumoDoc'
...
export async function GET(req: NextRequest) {
  ...
  const tipo = req.nextUrl.searchParams.get('tipo') === 'espelho' ? 'espelho' : 'resumo'

  const { data: run } = await sb.from('rh_fechamento_run')
    .select('id, competencia, ini, fim, rh_fechamento_run_linha(colaborador_id, nome, cpf, ini, fim, hn_min, he50_min, he100_min, faltas_min, total_min, quitacao_min)')
    .eq('id', runId).eq('org_id', org.id).maybeSingle()
  if (!run) return new Response('Fechamento não encontrado', { status: 404 })

  const linhas = (run.rh_fechamento_run_linha as (RunLinhaRef & FechResumoLinha)[])
  ...
  if (tipo === 'resumo') {
    pdf = await renderToBuffer(
      <FechamentoResumoDoc d={{ ini: run.ini, fim: run.fim, competencia: String(run.competencia).slice(0, 7), linhas }}
  ...
  const nome = `${tipo === 'resumo' ? 'Fechamento do ponto' : 'Espelho de ponto'} (${dBR(run.ini)} - ${dBR(run.fim)}).pdf`
