...
function ProfileForm({ user }: Props) {
  ...
  function restoreGoogle() {
    setFullName(user.googleName ?? '')
    setAvatarUrl(user.googleAvatar ?? '')
  }
        ...
        <div className="px-6 py-5">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              />
              <p className="text-[11px] text-gray-400 mt-1">O e-mail é gerenciado pela sua conta Google.</p>
            </div>
          </div>
        </div>
          ...
          <div className="px-6 py-4 bg-gray-50/60 rounded-b-xl">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-gray-500">
                Conta conectada via Google · dados importados automaticamente no primeiro acesso
              </span>
            </div>
          </div>
        )}
...
// ── Server wrapper to load user data ─────────────────────────────────────────
...
export default async function PerfilPage({
  ...
  // Google data comes from auth metadata
