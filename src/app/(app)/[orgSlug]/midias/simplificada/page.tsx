...
import { MidiasClient, type MidiaRow } from './MidiasClient'

export default async function MidiasSimplificadaPage({
  params,
  searchParams,
}: {
  ...
  const midias: MidiaRow[] = (raw ?? []).map((m: any) => ({
    ...
    desconto_pct: Number(m.desconto_pct ?? 0),
    faturamento: m.faturamento,
    situacao: m.situacao,
    archived: m.archived,
    cliente: m.workspaces?.name ?? '—',
