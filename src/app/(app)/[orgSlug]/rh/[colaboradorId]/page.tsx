...
export default async function ColaboradorPage({ params }: { params: Promise<{ orgSlug: string; colaboradorId: string }> }) {
  ...
  const colab = unwrapOne<Colaborador>(await (supabase as any)
    .from('rh_colaborador')
    .select('id, nome, cpf, email, telefone, cargo, tipo_vinculo, data_admissao, data_demissao, status, gestor_id, salario_atual, beneficios_mensal, custo_projetado_mensal, custo_overhead, aviso_previo_ini, aviso_previo_modo, observacao, arquivado, membro_user_id, bate_ponto, entra_fechamento')
