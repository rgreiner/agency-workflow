      ...
      {editing ? (
        <div className="border border-gray-200 rounded-xl p-3 space-y-2">
          {draft.map((r, i) => (
            <div key={i} className="flex items-center gap-2">
              <input value={r.label} onChange={e => setRow(i, 'label', e.target.value)} placeholder="Rótulo (ex.: Planejamento, Boletos)"
                className="w-40 shrink-0 text-sm border border-gray-200 rounded-lg px-2.5 py-1.5 focus:outline-none focus:ring-2 focus:ring-orange-500" />
              <input value={r.url} onChange={e => setRow(i, 'url', e.target.value)} placeholder="https://…"
                className="flex-1 min-w-0 text-sm border border-gray-200 rounded-lg px-2.5 py-1.5 focus:outline-none focus:ring-2 focus:ring-orange-500" />
          ...
          <div className="flex items-center justify-between pt-1">
            <button onClick={addRow} className="inline-flex items-center gap-1 text-xs text-orange-600 hover:text-orange-700 transition-colors">
        ...
        <div className="border border-gray-200 rounded-xl overflow-hidden divide-y divide-gray-100">
          {links.map((l, i) => (
            <a key={i} href={href(l.url)} target="_blank" rel="noopener noreferrer"
              ...
              <span className="flex-1 min-w-0 truncate text-xs text-orange-600 group-hover:underline">{l.url.replace(/^https?:\/\//, '')}</span>
