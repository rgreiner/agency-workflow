...
import { confirmReviewErrors, retryReview } from '@/app/actions/activity'
...
export function ReviewBanner({ activityId, path, status, errors, kind, currentStatus }: Props) {
  ...
  function retry() {
    start(async () => {
      const r = await retryReview(path, activityId)
      if (r?.error) toast.error(r.error)
      else toast.success('Revisão reenviada — o resultado aparece aqui em instantes.')
    })
  }
  ...
  // Falha do provider (ex.: 529 "Overloaded") costuma passar sozinha, então dá
  // pra pedir outra rodada sem mexer no status da tarefa.
  if (status === 'failed') {
    return (
      <div className="mt-4 flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-2.5 text-sm text-amber-700">
        ...
        <button
          ...
          onClick={retry}
          disabled={pending}
          className="shrink-0 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-semibold text-amber-800 bg-amber-100 hover:bg-amber-200 transition-colors disabled:opacity-50"
        >
