...
import { ChevronRight, Search, ArrowDownCircle, ArrowUpCircle } from 'lucide-react'
...
interface Grupo { contato: string; itens: AbertoItem[]; subtotal: number; maxAtraso: number }
...
export function InadimplentesClient({ itens, today }: { itens: AbertoItem[]; today: string }) {
  ...
  const grupos = useMemo(() => {
    ...
    for (const i of filtrados) {
      const g = map.get(i.contato) ?? { contato: i.contato, itens: [], subtotal: 0, maxAtraso: 0 }
      g.itens.push(i)
      g.subtotal += i.valor
