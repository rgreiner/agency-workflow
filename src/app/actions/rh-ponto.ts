...
/** Grava o motivo da extra pendente do dia — RPC estreita (mig. 255): não toca
 *  hora nem status, e só enquanto o gestor não decidiu. Só texto livre de
 *  propósito (o dia pode ter tido N tarefas; apontar uma distorceria). */
export async function salvarContextoExtra(orgSlug: string, colaboradorId: string, data: string, motivo: string) {
  const c = await ctx(orgSlug)
  if ('error' in c) return { error: c.error }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { error } = await (c.supabase as any).rpc('rh_extra_contexto', {
    p_colaborador_id: colaboradorId, p_data: data,
    p_motivo: motivo, p_projeto: null,
  })
  if (error) return { error: error.message }
  revalidatePath(`/${orgSlug}/ponto`)
  return { ok: true }
